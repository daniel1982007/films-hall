...
const AddFilmForm = () => {
    ...
    switch (fieldName) {
      case "Title":
        if (filmData.Title && !/^[\w:\-, ]+$/.test(filmData.Title)) {
          ...
          setTitleErrorMessage(
            "Title could be combination of letters, numbers, spaces or symbols : -"
          );
        }
      ...
      case "Stars":
        ...
        nameArr = nameArr.map((name) =>
          name.trim().toUpperCase().replace(/[ ]+/g, [])
        );
        ...
        if (filmData.Stars && !/^[a-zA-Z]+[a-zA-Z, ]+$/.test(filmData.Stars)) {
          ...
          setStarsErrorMessage(
            "Only letters, spaces and commas are allowed, and begins from a letter"
          );
        } else if (nameArr.some((name) => !name || !name.trim())) {
          setIsStarsErrorLabelOpen(true);
          setStarsErrorMessage(
            "Invalid names input, names are seperated by a comma, for example: Donald Trump, Joe Biden"
          );
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      isTitleErrorLabelOpen ||
      isYearErrorLabelOpen ||
      isStarsErrorLabelOpen
    ) {
      setHasErrorMessage(true);
  ...
  return (
    <div className="container-lg py-3">
      <CSSTransition
        ...
        unmountOnExit
      >
        <div className="alert alert-danger small mx-5 liveValidateMessage text-center">
          {errorMessage}
        </div>
      </CSSTransition>
      <div className="row g-4 m-0 align-items-center">
        <div className="col-md-6">
          <form onSubmit={handleSubmit}>
            <div className="my-2 mx-5">
              <label className="d-block form-label" htmlFor="title">
                Film Title
              </label>
              <input
            ...
            <div className="my-2 mx-5">
              <label className="d-block form-label" htmlFor="release-year">
                Release Year
              </label>
              <input
                className="form-control"
                ...
                id="release-year"
                type="text"
                placeholder="Release Year (for example: 2000)"
                onChange={handleChange}
            ...
            <div className="my-2 mx-5">
              <label className="d-block form-label" htmlFor="format">
                Video Format
              </label>
              <select
            ...
            <div className="my-2 mx-5">
              <label className="d-block form-label" htmlFor="actors">
                Casting
              </label>
              <input
                className="form-control"
                ...
                id="actors"
                type="text"
                placeholder="Film actors and actresses"
                onChange={handleChange}
        ...
        <div className="col-md-6">
          <div className="my-2 mx-5">
            ...
            <p className="h6 mt-3">
              If you want to add multiple films, you can import films from here
            </p>
          </div>
        </div>
      </div>
